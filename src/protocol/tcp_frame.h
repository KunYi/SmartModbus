/**
 * @file tcp_frame.h
 * @brief TCP/IP MBAP frame handling
 */

#ifndef SMARTMODBUS_TCP_FRAME_H
#define SMARTMODBUS_TCP_FRAME_H

#include <stdint.h>

#ifdef __cplusplus
extern "C" {
#endif

/**
 * @brief Build TCP frame with MBAP header
 * @param transaction_id Transaction ID
 * @param unit_id Unit ID (slave ID)
 * @param fc Function code
 * @param pdu_data PDU data
 * @param pdu_length PDU length
 * @param frame_buffer Output frame buffer
 * @param buffer_size Buffer size
 * @return Frame length on success, negative error code on failure
 */
int mb_tcp_build_frame(uint16_t transaction_id,
                       uint8_t unit_id,
                       uint8_t fc,
                       const uint8_t *pdu_data,
                       uint16_t pdu_length,
                       uint8_t *frame_buffer,
                       uint16_t buffer_size);

/**
 * @brief Parse TCP frame
 * @param frame_data Frame data
 * @param frame_length Frame length
 * @param transaction_id Output: transaction ID
 * @param unit_id Output: unit ID
 * @param fc Output: function code
 * @param pdu_data Output: PDU data buffer
 * @param pdu_length Output: PDU length
 * @return 0 on success, negative error code on failure
 */
int mb_tcp_parse_frame(const uint8_t *frame_data,
                       uint16_t frame_length,
                       uint16_t *transaction_id,
                       uint8_t *unit_id,
                       uint8_t *fc,
                       uint8_t *pdu_data,
                       uint16_t *pdu_length);

/**
 * @brief Calculate TCP frame length
 * @param pdu_length PDU length
 * @return Total frame length
 */
uint16_t mb_tcp_calc_frame_length(uint16_t pdu_length);

#ifdef __cplusplus
}
#endif

#endif  // SMARTMODBUS_TCP_FRAME_H
